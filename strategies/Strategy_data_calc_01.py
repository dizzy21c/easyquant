...
def do_init_data_buf(code, idx):
    ...
    print("do-init data data-buf size=%d " % len(data_buf_day))
...
class calcStrategy(Thread):
    ...
    def run(self):
        if self.last_time is None:
            self.last_time = pt.to_datetime(self._data['datetime'][0:10])
        else:
        ...
        df_day.loc(self.last_time).vol = self._data['vol']
        ...
        df_v=pd.concat([tdx.MA(df_day.vol, x) for x in (5,10,20,30,60,90,120,250,13, 34, 55,) ], axis = 1)[-1:]
        df_v.columns = [u'm5',u'm10',u'm20',u'm30',u'm60',u'm90',u'm120', u'm250', u'm13', u'm34', u'm55']

        # df_a=pd.concat([tdx.MA(df_day.amount, x) for x in (5,10,20,30,60,90,120,250,13, 34, 55,) ], axis = 1)[-1:]
