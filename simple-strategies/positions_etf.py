...
def do_main_work(code, data, log, positions):
    ...
    if now_price < hold_price / 1.05:
        log.info("code=%s now=%6.3f solding..." % (code, now_price))
        # 卖出
    now_vol = data['volume']
    ...
    df_day.loc[last_time,'open'] = data['open']
    df_day.loc[last_time,'high']= data['high']
    df_day.loc[last_time,'low'] = data['low']
    df_day.loc[last_time,'close'] = now_price
    df_day.loc[last_time,'vol'] = data['volume']
    df_day.loc[last_time,'amount'] = data['amount']
...
class calcStrategy(Thread):
    ...
    def run(self):
        ...
        ##TODO 绝对条件１
        if now_price < self.price / 1.03:
            self.log.info("idx code=%s now=%6.2f solding..." % (self.code, now_price))
        ...
        if now_price < df.m5.iloc[-1]:
            ...
            self.log.info("idx code=%s now=%6.2f pct=%6.2f m5=%6.2f, high=%6.2f, low=%6.2f" % (self.code, now_price, chag_pct, df.m5.iloc[-1], self._data['high'], self._data['low']))
