...
def udf_macd_zq(CLOSE,OPEN,HIGH,LOW):
  rtn = {'flg':False}
  len_d = len(CLOSE) - 1
  #macd zengqiang
  ...
  #DRAWTEXT(ER_JIN,0.4,'ER_JIN'),COLORYELLOW
  A1=BARSLAST(REF(CROSS(DIFF,DEA),1))
  #DI_BEI_LI=REF(CLOSE,A1+1)>CLOSE AND DIFF>REF(DIFF,A1+1) AND CROSS(DIFF,DEA),NODRAW

  # DI_BEI_LI=IFAND(IFAND(REF(CLOSE,A1+1)>CLOSE, DIFF>REF(DIFF,A1+1),True,False), CROSS(DIFF,DEA), True,False)
  REF(CLOSE,A1)
  ...
  # #DING_BEI_LI=REF(CLOSE,A2+1)<CLOSE AND REF(DIFF,A2+1)>DIFF AND CROSS(DEA,DIFF)
  # DING_BEI_LI=IFAND(IFAND(REF(CLOSE,A2+1)<CLOSE , REF(DIFF,A2+1)>DIFF, True,False) , CROSS(DEA,DIFF), True, False)
  ...
  # BEN_CI_LLV=LLV(LOW,BARSLAST(JC3<0))
  # BEN_CI_HHV=HHV(HIGH,BARSLAST(JC3>0))
