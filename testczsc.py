...
class ChanMain:
  ...
  def check_high(self, nCur, nHigh):
    dCur = self.data_df[nCur]['high']
    dHigh = self.data_df[nHigh]['high']
    if dCur >= dHigh:
      self.data_df[nHigh]['out'] = 0
      self.data_df[nCur]['out'] = 1
      nHigh = nCur
    return nHigh
  ...
  def get_df(data_df, index, col="high"):
    return data_df.loc[index][key]
  ...
  def pd_parse1(data_df): #nCount,  pOut,  pHigh,  pLow):
    ...
    for index, row in data_df.iterrows():
      ...
      if row['high'] >= self.get_df_high(data_df, nHigh):
        # nHigh = index
        # data_df.loc[nHigh] = 1
